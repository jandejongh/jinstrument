...
implements SignalGenerator
{
  ...
  {
    super ("HP-8116A", device, null, null, false, false, false, false /* XXX */ /* true, true, true, false */);
  }
  ...
  private final static ReadlineTerminationMode READLINE_TERMINATION_MODE = ReadlineTerminationMode.CR_LF;
    ...
    throws IOException, InterruptedException, TimeoutException
  {
    ...
    if (l1.length != HP8116A_GPIB_Instrument.L1_LENGTH)
    {
      LOG.log (Level.WARNING, "Unexpected number of bytes read for L1: {0} (should be {1}).",
        new Object[]{l1.length, HP8116A_GPIB_Instrument.L1_LENGTH});
    ...
    if (this.lastL1 != null)
      for (int i = 0; i < HP8116A_GPIB_Instrument.L1_LENGTH; i++)
    ...
    this.lastL1 = l1;
    return HP8116A_GPIB_Instrument.settingsFromL1 (l1);
  }
