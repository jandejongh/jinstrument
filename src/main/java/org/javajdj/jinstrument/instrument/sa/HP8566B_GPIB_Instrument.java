  ...
  implements SpectrumAnalyzer
{
  ...
  {
    super ("HP-8663A", device, null, null, /* XXX */ false, true, true, true);
  }
    ...
    throws InterruptedException, IOException, TimeoutException
  {
    final byte[] bytes = ((GpibDevice) getDevice ()).writeAndReadlnSync (string.getBytes (Charset.forName ("US-ASCII")),
      ...
      HP8566B_GPIB_Instrument.READLINE_TIMEOUT_MS);
  ...
  {
    final GpibControllerCommand getSettingsCommandOL = ((GpibDevice) getDevice ()).generateWriteAndReadNCommand (
      "OL;\n".getBytes (Charset.forName ("US-ASCII")),
  ...
  @Override
  public synchronized SpectrumAnalyzerSettings getSettingsFromInstrumentSync ()
    throws IOException, InterruptedException, TimeoutException
  {
    ...
    final byte[] ol = (byte[]) getSettingsCommand.get (GpibControllerCommand.CCARG_GPIB_READ_BYTES);
    ...
    throws IOException, InterruptedException, TimeoutException
  {
    ...
    ((GpibDevice) getDevice ()).atomicSequenceSync (
      ...
      HP8566B_GPIB_Instrument.READLINE_TIMEOUT_MS);
    final byte[][] bytes = (byte[][]) getTraceCommand.get (GpibControllerCommand.CCARG_GPIB_READ_BYTES);
    final String[] strings = new String[bytes.length];
    for (int s = 0; s < strings.length; s++)
      strings[s] = new String (bytes[s], Charset.forName ("US-ASCII"));
    final byte[] ol = (byte[]) getSettingsCommand.get (GpibControllerCommand.CCARG_GPIB_READ_BYTES);
  ...
  private static InstrumentReading traceFromO3 (final String[] strings, final SpectrumAnalyzerSettings settings)
  {
    ...
    return new DefaultSpectrumAnalyzerTrace (settings, samples, true, NAME_PROPERTY_NAME, true, true);
  }
    ...
    try
    {
      switch (commandString)
      {
        ...
        case InstrumentCommand.IC_RESOLUTION_BANDWIDTH:
        {
          ...
          writeAsync ("RB " + resolutionBandwidth_Hz + " HZ;\n");
          break;
        }
        ...
        case InstrumentCommand.IC_VIDEO_BANDWIDTH:
        {
          ...
          writeAsync ("VB " + videoBandwidth_Hz + " HZ;\n");
          break;
        }
        ...
        case InstrumentCommand.IC_REFERENCE_LEVEL:
        {
          ...
          writeAsync ("RL " + referenceLevel_dBm + " DM;\n");
          break;
        }
        case InstrumentCommand.IC_RF_ATTENUATION:
        {
          ...
          writeAsync ("AT " + rfAttenuation_dB + " DB;\n");
          break;
        }
