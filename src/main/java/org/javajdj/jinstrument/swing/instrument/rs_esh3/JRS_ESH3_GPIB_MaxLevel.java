 ...
 * 
 */
public class JRS_ESH3_GPIB_MaxLevel
  ...
  implements InstrumentView
{
  ...
  private static final Logger LOG = Logger.getLogger (JRS_ESH3_GPIB_MaxLevel.class.getName ());
    ...
    final Color panelColor)
  {
    ...
    this.jLevelSlider_dB.setValue (200);
  ...
  public JRS_ESH3_GPIB_MaxLevel (final RS_ESH3_GPIB_Instrument selectiveLevelMeter, final int level)
  {
    this (selectiveLevelMeter, null, level, null);
  }
  
  public JRS_ESH3_GPIB_MaxLevel (final RS_ESH3_GPIB_Instrument selectiveLevelMeter)
  {
  ...
  public final static InstrumentViewType INSTRUMENT_VIEW_TYPE = new InstrumentViewType ()
  {
    ...
    {
      return "R&S ESH-3 [GPIB] Maximum Level Settings";
    }
    ...
    public final InstrumentView openInstrumentView (final Instrument instrument)
    {
      if (instrument != null && (instrument instanceof RS_ESH3_GPIB_Instrument))
        return new JRS_ESH3_GPIB_MaxLevel ((RS_ESH3_GPIB_Instrument) instrument);
      else
        return null;
    }
  ...
  {
    return JRS_ESH3_GPIB_MaxLevel.INSTRUMENT_VIEW_TYPE.getInstrumentViewTypeUrl ()
      + "<>"
  ...
  private final MouseListener jLevelMouseListener = new MouseAdapter ()
  {
    ...
    public void mouseClicked (final MouseEvent me)
    {
      ...
      if (newLevel_dB != null)
        try
        {
          ((RS_ESH3_GPIB_Instrument) JRS_ESH3_GPIB_MaxLevel.this.getSelectiveLevelMeter ()).setMaximumLevel_dB (newLevel_dB);
        }
        catch (InterruptedException ie)
        {
           LOG.log (Level.INFO, "Caught InterruptedException while setting maximum level on instrument: {0}.",
             ie.getStackTrace ());
        }
        catch (IOException ioe)
        {
           LOG.log (Level.INFO, "Caught IOException while setting maximum level on instrument: {0}.",
             ioe.getStackTrace ());
        }
    }
  };
  ...
  private final ChangeListener jLevelSlider_ChangeListener = (final ChangeEvent ce) ->
  {
    ...
    {
      if (! JRS_ESH3_GPIB_MaxLevel.this.inhibitInstrumentControl)
        try
        {
          ((RS_ESH3_GPIB_Instrument) JRS_ESH3_GPIB_MaxLevel.this.getSelectiveLevelMeter ()).setMaximumLevel_dB (
            source.getValue ());
        }
        catch (IOException | InterruptedException e)
        {
          LOG.log (Level.INFO, "Caught exception while setting maximum level from slider to {0} dB: {1}.",
  ...
  private final InstrumentListener instrumentListener = new InstrumentListener ()
  {
    ...
    {
      if (instrument != JRS_ESH3_GPIB_MaxLevel.this.getSelectiveLevelMeter ()|| instrumentSettings == null)
        throw new IllegalArgumentException ();
      ...
      {
        JRS_ESH3_GPIB_MaxLevel.this.inhibitInstrumentControl = true;
        //
        try
        {
          final boolean maxMinMode = settings.getMaxMinMode ();
          final double maximumLevel_dB = settings.getMaximumLevel_dB ();
          if (maxMinMode)
          {
            JRS_ESH3_GPIB_MaxLevel.this.jLevel.setNumber (maximumLevel_dB);
            JRS_ESH3_GPIB_MaxLevel.this.jLevelSlider_dB.setValue ((int) Math.round (maximumLevel_dB));
            JRS_ESH3_GPIB_MaxLevel.this.jLevelSlider_dB.setToolTipText (
              Integer.toString ((int) Math.round (maximumLevel_dB)));
          }
          else
          {
            JRS_ESH3_GPIB_MaxLevel.this.jLevel.setBlank ();
            JRS_ESH3_GPIB_MaxLevel.this.jLevelSlider_dB.setValue (200);
            JRS_ESH3_GPIB_MaxLevel.this.jLevelSlider_dB.setToolTipText ("-");
          }
        }
        finally
        {
          JRS_ESH3_GPIB_MaxLevel.this.inhibitInstrumentControl = false;
        }
      });
    }
