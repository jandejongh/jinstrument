...
/** A Swing panel for the RF Attenuation settings of a {@link RS_ESH3_GPIB_Instrument} Selective Level Meter.
 *
 * @author Jan de Jongh {@literal <jfcmdejongh@gmail.com>}
 * 
 */
public class JRS_ESH3_GPIB_RF_Attenuation
  ...
  implements InstrumentView
{
  ...
  private static final Logger LOG = Logger.getLogger (JRS_ESH3_GPIB_RF_Attenuation.class.getName ());
  ...
  public JRS_ESH3_GPIB_RF_Attenuation (
    ...
    final Color panelColor)
  {
    ...
    this.jAtt = new JSevenSegmentNumber (JInstrumentPanel.getGuiPreferencesAmplitudeColor (), false, 5, 2);
    this.jAtt.addMouseListener (this.jAttMouseListener);
    add (this.jAtt);
    this.jAttSlider_dB = new JSlider (0, 140, 10);
    this.jAttSlider_dB.setToolTipText ("-");
    this.jAttSlider_dB.setMajorTickSpacing (20);
  ...
  public JRS_ESH3_GPIB_RF_Attenuation (final RS_ESH3_GPIB_Instrument selectiveLevelMeter, final int level)
  {
    this (selectiveLevelMeter, null, level, null);
  }
  
  public JRS_ESH3_GPIB_RF_Attenuation (final RS_ESH3_GPIB_Instrument selectiveLevelMeter)
  {
  ...
  public final static InstrumentViewType INSTRUMENT_VIEW_TYPE = new InstrumentViewType ()
  {
    ...
    {
      return "R&S ESH-3 [GPIB] RF Attenuation Settings";
    }
    ...
    public final InstrumentView openInstrumentView (final Instrument instrument)
    {
      if (instrument != null && (instrument instanceof RS_ESH3_GPIB_Instrument))
        return new JRS_ESH3_GPIB_RF_Attenuation ((RS_ESH3_GPIB_Instrument) instrument);
      else
        return null;
    }
  ...
  {
    return JRS_ESH3_GPIB_RF_Attenuation.INSTRUMENT_VIEW_TYPE.getInstrumentViewTypeUrl ()
      + "<>"
  ...
  private final MouseListener jAttMouseListener = new MouseAdapter ()
  {
    ...
    {
      final Double newAttenuation_dB = JInstrumentDialog.getPowerRatioFromDialog_dB ("Enter RF Attenuation [dB]", null);
      if (newAttenuation_dB != null)
        try
        {
          ((RS_ESH3_GPIB_Instrument) JRS_ESH3_GPIB_RF_Attenuation.this.getSelectiveLevelMeter ()).setRfAttenuation_dB
            (newAttenuation_dB);
        }
  ...
  private final ChangeListener jAttSlider_ChangeListener = (final ChangeEvent ce) ->
  {
    ...
    {
      if (! JRS_ESH3_GPIB_RF_Attenuation.this.inhibitInstrumentControl)
        try
        {
          ((RS_ESH3_GPIB_Instrument) JRS_ESH3_GPIB_RF_Attenuation.this.getSelectiveLevelMeter ()).setRfAttenuation_dB (
            source.getValue ());
        ...
        {
          LOG.log (Level.INFO, "Caught exception while setting RF Attenuation from slider to {0} dB: {1}.",
  ...
  private final InstrumentListener instrumentListener = new InstrumentListener ()
  {
    ...
    {
      if (instrument != JRS_ESH3_GPIB_RF_Attenuation.this.getSelectiveLevelMeter ()|| instrumentSettings == null)
        throw new IllegalArgumentException ();
      ...
      {
        JRS_ESH3_GPIB_RF_Attenuation.this.inhibitInstrumentControl = true;
        //
        try
        {
          final RS_ESH3_GPIB_Settings.AttenuationMode attenuationMode = settings.getAttenuationMode ();
          final double rfAttenuation_dB = settings.getRfAttenuation_dB ();
          switch (attenuationMode)
          {
            case AutoRangingLowNoise:
            case AutoRangingLowDistortion:
              JRS_ESH3_GPIB_RF_Attenuation.this.jAtt.setBlank ();
              JRS_ESH3_GPIB_RF_Attenuation.this.jAttSlider_dB.setValue (140);
              JRS_ESH3_GPIB_RF_Attenuation.this.jAttSlider_dB.setToolTipText ("-");
              break;
            case Manual:
              JRS_ESH3_GPIB_RF_Attenuation.this.jAtt.setNumber (rfAttenuation_dB);
              JRS_ESH3_GPIB_RF_Attenuation.this.jAttSlider_dB.setValue ((int) Math.round (rfAttenuation_dB));
              JRS_ESH3_GPIB_RF_Attenuation.this.jAttSlider_dB.setToolTipText (
                Integer.toString ((int) Math.round (rfAttenuation_dB)));
              break;
            default:
        ...
        {
          JRS_ESH3_GPIB_RF_Attenuation.this.inhibitInstrumentControl = false;
        }
      });
    }
